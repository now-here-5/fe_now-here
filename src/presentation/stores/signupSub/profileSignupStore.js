...
export const profileSignupStore = defineStore(
  'profileSignup',
  () => {
    const name = ref('')
    ...
    const isSelfIntroductionValid = computed(
      () => selfIntro.value.length >= 10 && selfIntro.value.length <= 30
    )
    ...
    watch(signupReady, (newValue) => {
      ...
      if (newValue) {
        store_Signup.signupCompleted.profile = true
      } else {
        store_Signup.signupCompleted.profile = false
      }
    })
